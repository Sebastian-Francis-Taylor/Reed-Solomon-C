...
static void reverse_bytes(uint8_t *arr, int len) {
...
int main() {
    ...
    uint8_t *test_arr_2 = calloc(223, sizeof(uint8_t));
    for (int i = 0; i < info_len; ++i) {
        test_arr_2[i] = i;
    }

    uint8_t *encoded_message = rs_encode(test_arr_2, info_len);
    int encoded_len = 255;
    ...
    printf("\n");
    */
    reverse_bytes(encoded_message, 255);
